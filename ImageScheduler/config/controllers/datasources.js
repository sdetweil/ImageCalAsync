...
//var common  = require('/home/pi/smart-mirror/plugins/ImageScheduler/common')
...
module.exports.delete = function datasourcesDELETE (req, res, next) {
	...
	**/
	...
	{
		common.getdb().collection('DataSources').findOne({_id: ObjectId(parms.id) },
		function(err,result)
					{
						if(result)
						{
							common.getdb().collection('DataSources').remove({_id: ObjectId(parms.id) }, 
									function(err,result)
									{
										if(err)
										{
...
module.exports.get = function datasourcesGET (req, res, next) {
	...
	**/
	...
	if(parms.id.length==12 || parms.id.length==24)
	{
		...
		common.getdb().collection('DataSources').find({_id: ObjectId(parms.id)}).toArray(function(err,items)
		{
						res.statusCode = 200;
	...
	res.end(JSON.stringify(badid || {}, null, 2));
			...
			{
				common.getdb().collection('DataSources').find().toArray(function(err,items)
				{
						res.statusCode = 200;
						res.end(JSON.stringify(items || {}, null, 2));
				});
...
module.exports.post = function datasourcesPOST (req, res, next) {
	...
	**/
	...
	if(Object.keys(examples).length > 0) 
	{
		...
		var body=req.body;
		common.getdb().collection('DataSources').findOne({"Name": body.Name },
		function(err,result)
					{
						if(result)
						{
						...
						{		
							...
							common.getdb().collection('DataSources').insertOne(body, 
									function(err,result)
									{
										if(err)
										{
...
module.exports.put = function datasourcesPUT (req, res, next) {
	...
	**/
	...
	if(parms.id.length==12 || parms.id.length==24)
	{
		...
		common.getdb().collection('DataSources').findOne({_id: parms.id },
		function(err,result)
					{
						if(result)
						{
						...
						{
							common.getdb().collection('DataSources').update({_id: ObjectId(parms.id) },body, 
									function(err,result)
									{
										if(err)
										{
